...
{
    //from  https://jhvirtualkeyboard.codeplex.com/documentation
    public static class StringLib
    {
        ...
        {
            if (HasNothing(pathReference))
            {
                throw new ArgumentNullException("pathReference");
            }
            if (HasNothing(pathToTest))
            {
        ...
        public static string TopOfFilesystemPath(string filesystemFolder, out string pathRemainder)
        {
            string result = null;
            pathRemainder = null;
            if (HasSomething(filesystemFolder))
            {
                string path = filesystemFolder.Trim();
                if (HasSomething(path))
                {
        ...
        public static bool Matches(this string sourceText, string pattern)
        {
            ...
            if (sourceText.Equals(pattern, StringComparison.InvariantCultureIgnoreCase))
            {
                return true;
            }
            else
        ...
        {
            if (HasSomething(filesystemPath))
            {
                string path = filesystemPath.Trim();
                if (path.Length >= 2 && IsEnglishAlphabetLetter(path[0]) && path[1] == ':')
                {
                    return true;
                }
            }
        ...
        /// <returns>true if any of the patterns (or plural forms) is found</returns>
        static public bool TestIfContainsAndRemove(this string thisText, string word1, string word2, string word3, string word4, out string thisTextWithoutThat)
        {
            bool wasFound = false;
            string sWordFound;
        ...
        /// <param name="isForDeveloper">if true, then show everything - including that which only a developer would care about</param>
        /// <returns>a string that describes the given exception</returns>
        public static string TextForException(Exception exception, bool isForDeveloper)
        {
            var sb = new StringBuilder();
            if (isForDeveloper)
            {
                sb.Append(exception.ToString());
                // Write out the Data of this and any inner exceptions!
                Exception x = exception;
                while (x != null)
                {
                    // Write out any information that is in the Data dictionary of this exception.
                    sb.Append(TextOfExceptionDataDictionary(x));
                    x = x.InnerException;
                }
            }
            else
            {
                // Write out the Message property of this exception plus that of any inner exceptions..
                sb.AppendLine(exception.GetType().ToString());
                sb.Append(exception.Message);
                sb.Append(TextOfExceptionDataDictionary(exception));
                Exception x = exception;
        ...
        private static string TextOfExceptionDataDictionary(Exception exception)
        {
            ...
            if (exception.Data.Count > 0)
            {
                ...
                foreach (var dataKey in exception.Data.Keys)
                {
                    ...
                    if (!(keyString != null && keyString.StartsWith("HelpLink")))
                    {
                        ...
                        sb.Append(dataKey);
                        sb.Append(": ");
                        sb.Append(exception.Data[dataKey]);
                        isFirst = false;
                    }
                }
                return sb.ToString();
        ...
        public static string ToPlural(this string singularWord, CultureInfo cultureInfo)
        {
            ...
            {
                throw new ArgumentNullException("singularWord");
            }
        ...
        /// <returns></returns>
        /// <exception cref="ArgumentNullException"/>
        public static string WithPlurality(this int nQuantity, string sUnitName)
        {
            if (sUnitName == null)
            {
                throw new ArgumentNullException("sUnitName");
            }
        ...
        /// <exception cref="ArgumentNullException"/>
        /// <exception cref="ArgumentException"/>
        public static string WithPlurality(this int nQuantity, string sUnitNameSingular, string sUnitNamePlural)
        {
            if (sUnitNameSingular == null)
            {
                throw new ArgumentNullException("sUnitNameSingular");
            }
            if (sUnitNamePlural == null)
            {
                throw new ArgumentNullException("sUnitNamePlural");
            }
            if (HasNothing(sUnitNamePlural))
            {
