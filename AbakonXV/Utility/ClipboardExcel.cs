...
{
    #region Thanks to Alexey Merson :  http://www.codeproject.com/Articles/149009/Getting-the-Excel-Range-object-from-the-Clipboard
    ...
    public class RangeName
    {
        ...
        {
            string[] indices = R1C1Name.Split('W', 'R', 'K', 'C');
            if (indices.Length == 3)
            {
                int columnNumber = int.Parse(indices[2]);
                return GetColumnName(columnNumber) + indices[1];
            }
            else if (indices.Length == 2 && (R1C1Name.Contains("W") || R1C1Name.Contains("R")))
            {
                int columnNumber = int.Parse(indices[1]);
                return GetColumnName(columnNumber);
            }
            else if (indices.Length == 2 && (R1C1Name.Contains("K") || R1C1Name.Contains("C")))
            {
                return indices[1];
            }
            else
    ...
    public static class ClipboardExcel
    {
        ...
        private static IMoniker IMonikerFromIStream(IStream iStream)
        {
            ...
            Guid guid = Marshal.GenerateGuidForType(typeof(IUnknown));
            object obj;
