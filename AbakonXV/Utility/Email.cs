...
namespace AbakonXVWPF.Utility
{
    ...
    class MAPI
    {
        ...
        int SendMail(string strSubject, string strBody, int how)
        {
            ...
            StringBuilder erStringBuilder = new StringBuilder();

            if (m_lastError > 1)
                //     MessageBox.Show("MAPISendMail failed! " + GetLastError(), "MAPISendMail");
                erStringBuilder.AppendLine("MAPISendMail failed! " + GetLastError());
            Cleanup(ref msg);
            return m_lastError;
        }
        ...
        IntPtr GetAttachments(out int fileCount)
        {
            ...
            {
                mapiFileDesc.name = System.IO.Path.GetFileName(strAttachment);
